...
impl TabletSink {
    ...
    pub fn with_extent(name: &str, max_x: i32, max_y: i32) -> Result<Self> {
        ...
        // **No wheel axes here.** Attaching them was tried and does not work: libinput
        // classifies a device as a tablet tool *or* a pointer and never both, so a tablet's
        // relative axes are routed into the tablet-tool interface as an airbrush finger wheel
        // rather than becoming pointer scroll. Measured — KWin reports `pointer = false` on
        // this device however many relative axes it declares (P9). Scrolling in tablet
        // transport is handled by freezing the pen instead; see the daemon's runtime.
